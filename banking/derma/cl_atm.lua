...
function PANEL:Init()
    ...
    self:SetAlpha(0)
    self:AlphaTo(255, 0.3, 0, function(alpha, panel)
        panel:SetAlpha(255)
    end)
end
...
function PANEL:Populate()
        ...
        [1] = {
          ...
          Exe = function(btn)
            if !btn.disabled then
                if !self.TypeBG or !self.TypeBG:IsValid() then
                    ...
                    self.TypeAgree.DoClick = function(btn)
                        ...
                        if !ply.execCD or CurTime() >= ply.execCD then
                          net.Start('bankeer::exec', "bank_withdrawmoney", amount)
                  ...
                  self.TypeBG:AlphaTo(0, .2, 0, function(anim, panel) 
                      panel:Remove()
                  end)
    ...
    for k, v in pairs(funcs) do
      ...
      self.btnList[k]:Dis(v.Disable and v.Disable())
      self.btnList[k].DoClick = function(btn)
          if !btn.Click or CurTime() > btn.Click then
            v.Exe(btn)
            btn.Click = CurTime() + 1;
          end;
      end;
